...
var
  ...
  IPSBitmap: TBitmap;
  Controller, LoadInjection, G1, G2, PT0, xt, zt: TIPSClass;
  cx, ce, cv, cy, cys, cyr, cz: TConnectionClass;
  DrawColour: TColor;
begin
  ...
  try
    ...
    PT0 := TPT0Class.Create;
    PT0.blockDiagram := BlockDiagram;
    SetRect(PT0.boundsRect, 190, 180, 250, 216);
    PT0.title := '';
    PT0.font.Style := [fsItalic];
    PT0.Draw;
    cys.Next := PT0;

    cv := TConnectionClass.Create;
    cv.blockDiagram := BlockDiagram;
    cv.sourceObject := LoadInjection;
    cv.sourceAnchor := bottommiddle;
    cv.drainObject := PT0;
    cv.drainAnchor := rightmiddle;
    cv.chirality := cright;
    cv.Draw;
    PT0.Next := cv;
