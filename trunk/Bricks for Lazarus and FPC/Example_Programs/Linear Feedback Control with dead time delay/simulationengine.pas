...
type
  ...
  public
    x, z, e, v, y, yr, ys: array of extended;
    constructor Create;
    destructor Destroy;
...
procedure RunSimulation(x, z, G1, G2: extended; T0: longint; nmax: integer);

implementation

procedure RunSimulation(x, z, G1, G2: extended; T0: longint; nmax: integer);
var
  e, v, y, yr, ys: extended;
  i: integer;
begin
  if nmax > 0 then
  begin
    gPrediction.x := x;
    gPrediction.z := z;
    gPrediction.v := (G1 * x + z) / (1 + G1 * G2);
    gPrediction.y := gPrediction.v;
    ...
    begin
      ...
      v := gBlocks.LoadInjection.simOutput;
      gBlocks.T0.input := v;
      y := gBlocks.T0.simOutput;
      gBlocks.G2.input := y;
